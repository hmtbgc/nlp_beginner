...
class CustomDataset(data.Dataset):       
    def __init__(self, datas, labels, length, words):
        self.datas = datas
        self.labels = labels
        self.length = length
        self.words = words
    ...
    def __getitem__(self, index):
        sent, label, leng, word = self.datas[index], self.labels[index], self.length[index], self.words[index]
        return sent, label, leng, word
...
def numpy_torch(a, b, c, d):
    a, b, c, d = torch.tensor(a, dtype=torch.long), torch.tensor(b, dtype=torch.long), \
              torch.tensor(c, dtype=torch.long), torch.tensor(d, dtype=torch.long)
    return a, b, c, d

def print_to_file(model, datasets, length, chars, word, label, result_path):
    paths = []
    m = datasets.shape[0] // Batch_size + 1
    for i in range(m):
        begin = i * Batch_size
        end = (i + 1) * Batch_size
        if end > datasets.shape[0]:
            end = datasets.shape[0]
        batch_datasets = datasets[begin : end].to(device)
        batch_length = length[begin : end].to(device)
        batch_chars = chars[begin : end].to(device)
        _, path = model(batch_datasets, batch_length, batch_chars)
        path = list(itertools.chain.from_iterable(path))
        paths.append(path)
        batch_datasets = batch_datasets.cpu()
        batch_length = batch_length.cpu()
        batch_chars = batch_chars.cpu()
        del batch_datasets, batch_length, batch_chars, path
...
def init_embedding(input_embedding):
    x = np.sqrt(3.0 / input_embedding.size(1))
    nn.init.uniform_(input_embedding, -x, x)  
...
class BiLSTM_CRF(nn.Module):
    def __init__(self, filter_size, filter_num, char_vocab, char_emb_dim, vocab_size, label_to_id, embedding_dim, seq_len, hidden_dim, batch_size, weights, dropout):
        ...
        self.char_emb = nn.Embedding(len(self.char_vocab), self.char_emb_dim)
        init_embedding(self.char_emb.weight)

        self.char_conv = []
        for filter_size in self.filter_size:
            self.char_conv.append(nn.Conv3d(1, self.filter_num, (1, filter_size, self.char_emb_dim), bias=True))
        
        self.embedding = nn.Embedding(self.vocab_size, self.embedding_dim)
        self.embedding.weight.data.copy_(self.weights)

        self.lstm = nn.LSTM(self.embedding_dim + self.char_emb_dim, self.hidden_dim // 2, num_layers=1, \
    ...
    def char_forward(self, inputs):
        # inputs : batch, seq_len, max_char_len
        batch, max_len, char_len = inputs.size(0), inputs.size(1), inputs.size(2)
        x = inputs.view(batch, max_len * char_len)
        
        x = self.char_emb(x)
        # x : batch, maxlen * char_len, char_emb_dim 
        x = x.view(-1, 1, max_len, char_len, self.char_emb_dim)
        char_conv = []
        for conv in self.char_conv:
            conv = conv.to(device)
            out = conv(x)
            out = torch.squeeze(torch.max(out, -2)[0], -1)
            out = torch.squeeze(out, -1)
            char_conv.append(out)
        char_conv = torch.cat(char_conv, dim=1)
        char_conv = char_conv.permute(0, 2, 1)
        return char_conv
        
    def get_feature(self, sent, word):
        # sent : batch, seq_len
        # word : batch, seq_len, max_char_len
        char_conv = self.char_forward(word)
        char_conv = self.dropout_layer(char_conv)
    
        sent_emb = self.embedding(sent).view(-1, self.seq_len, self.embedding_dim)
        embeds = torch.cat((sent_emb, char_conv), -1)
        embeds = self.dropout_layer(embeds)
        # embeds : batch, seq_len, embedding_dim + char_emb_size
    ...
    def neg_log_likelihood(self, sents, labels, length, words):
        feats = self.get_feature(sents, words).to(device)
    ...
    def forward(self, sents, length, words):
        feats = self.get_feature(sents, words)
        scores = []
        paths = []
...
x = pickle.load(open('../input/params-for-ner-iobes-cnn/params_for_ner_iobes_cnn.pkl', 'rb'), encoding='UTF-8')
...
train_char = x['train_char']
val_char = x['val_char']
test_char = x['test_char']
char_all = x['char_all']
char_to_id = x['char_to_id']
id_to_char = x['id_to_char']
char_to_id['~'] = 0
id_to_char[0] = '~'
char_all.insert(0, '~')
...
train_dataset = CustomDataset(train_num, train_label_num, train_len, train_char)
train_generator = data.DataLoader(train_dataset, **params)

net = BiLSTM_CRF(Filter_size, Filter_num, char_all, char_dim, vocab_size, label_to_id, Embedding_size, \
...
best_f1 = 0.
index = 0
train_num, train_label_num, train_len, train_char = numpy_torch(train_num, train_label_num, train_len, train_char)
val_num, val_label_num, val_len, val_char = numpy_torch(val_num, val_label_num, val_len, val_char)
global_step = 0

for epoch in range(Epoch):
    for i, (batch_train, batch_label, batch_len, batch_char) in enumerate(train_generator):
        net.train(mode=True)
        global_step += 1
        batch_train = batch_train.long().to(device)
        batch_len = batch_len.long().to(device)
        batch_label = batch_label.long().to(device)
        batch_char = batch_char.long().to(device)
        loss = net.neg_log_likelihood(batch_train, batch_label, batch_len, batch_char)
        ...
        if i % 20 == 0:
            ...
            print('learning_rate:{:.5f}'.format(current_lr))
        optimizer.zero_grad()
        loss.backward()
        nn.utils.clip_grad_norm_(net.parameters(), 5)
        optimizer.step()
        batch_train, batch_label, batch_len, batch_char = gpu_cpu(batch_train, batch_label, batch_len, batch_char)
        del batch_train, batch_label, batch_len, batch_char, loss
        if global_step % 600 == 0:
            ...
            with torch.no_grad(): 
                net.eval()
                #train_num, train_label_num, train_len, train_char = cpu_gpu(train_num, train_label_num, train_len, train_char)
                print('gpu is ok to hold on!!')
                out1 = print_to_file(net, train_num, train_len, train_char, train_word, train_tag, result_path='train_result{}.txt'.format(index))
                p1, r1, f1_1 = evaluate(out1)
                with open('train_p_r_f1.txt', 'a') as fff:
                    fff.write(str(p1) + ' ' + str(r1) + ' ' + str(f1_1) + '\n')
                del p1, r1, f1_1, out1
                #train_num, train_label_num, train_len, train_char = gpu_cpu(train_num, train_label_num, train_len, train_char)
                #val_num, val_label_num, val_len, val_char = cpu_gpu(val_num, val_label_num, val_len, val_char)
                out2 = print_to_file(net, val_num, val_len, val_char, val_word, val_tag, result_path='val_result{}.txt'.format(index))
                ...
                del p2, r2, f1_2, out2
                #val_num, val_label_num, val_len, val_char = gpu_cpu(val_num, val_label_num, val_len, val_char)

...
def print_to_file(model, datasets, length, word, label, result_path, lr):
    paths = model(datasets, length)
    paths = list(itertools.chain.from_iterable(paths))
    out = []
    with open(result_path, 'a') as f:
        f.write('learning_rate:{:.3f}\n'.format(lr))
...
class BiLSTM(nn.Module):
    ...
    """
    def get_loss(self, sents, length, labels):
        loss = 0
        criterion = nn.CrossEntropyLoss()
        feats = self.get_feature(sents)
        for feat, leng, label in zip(feats, length, labels):
            feat = feat[:leng]
            label = label[:leng]
...
net = BiLSTM(vocab_size, label_to_id, Embedding_size, \
...
for epoch in range(Epoch):
    for i, (batch_train, batch_label, batch_len) in enumerate(train_generator):
        ...
        loss = net.get_loss(batch_train, batch_len, batch_label)
        ...
        if global_step % 500 == 0:
            ...
            if global_step > Epoch * len(train_generator) // 3:
                current_lr = current_lr / (1 + decay_rate * index)
                adjust_learning_rate(optimizer, current_lr)
            with torch.no_grad(): 
                net.eval()
                train_num, train_label_num, train_len = cpu_gpu(train_num, train_label_num, train_len)
                out1 = print_to_file(net, train_num, train_len, train_word, train_tag, result_path='train_result{}.txt'.format(index), lr=current_lr)
                ...
                out2 = print_to_file(net, val_num, val_len, val_word, val_tag, result_path='val_result{}.txt'.format(index), lr=current_lr)
